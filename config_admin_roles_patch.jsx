...
(function () {
  ...
  async function downloadCredentialPdf(user) {
    try {
      ...
      pdf.setFillColor(31, 76, 180);
      pdf.roundedRect(0, 0, 85.6, 15, 2.8, 2.8, 'F');
      pdf.setFillColor(71, 105, 210);
      pdf.circle(77, 0, 15, 'F');
      pdf.setFillColor(255, 255, 255);
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(15);
      pdf.text('PIAGET', 5, 10.5);
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(5.6);
      pdf.text('CREDENCIAL ADMINISTRATIVA', 5, 4.6);
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(5.8);
      pdf.text(active ? 'ACTIVA' : 'NO ACTIVA', 70.5, 8.8);

      const px = 5, py = 19, pw = 20, ph = 24;
      pdf.setFillColor(241, 245, 249);
      pdf.roundedRect(px, py, pw, ph, 2.2, 2.2, 'F');
      if (avatar) {
        try { pdf.addImage(avatar, avatar.startsWith('data:image/png') ? 'PNG' : 'JPEG', px, py, pw, ph, undefined, 'FAST'); }
        catch (_) { pdf.text(initials(name), px + 6, py + 13); }
      } else {
        pdf.setTextColor(31, 76, 180);
        pdf.setFont('helvetica', 'bold');
        pdf.setFontSize(12);
        pdf.text(initials(name), px + 5.6, py + 13.2);
      }

      pdf.setTextColor(17, 24, 39);
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(9.2);
      const nameLines = pdf.splitTextToSize(name, 37);
      pdf.text(nameLines.slice(0, 2), 29, 22);
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(5.6);
      pdf.setTextColor(71, 85, 105);
      pdf.text(String(role).toUpperCase(), 29, nameLines.length > 1 ? 30.6 : 27.2);
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(4.7);
      pdf.setTextColor(100, 116, 139);
      pdf.text('CORREO', 29, 34.8);
      pdf.text('ID', 29, 43.1);
      pdf.text('TIPO', 45, 43.1);
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(5.4);
      pdf.setTextColor(17, 24, 39);
      pdf.text(pdf.splitTextToSize(email, 34).slice(0, 2), 29, 37.2);
      pdf.setFont('courier', 'normal');
      pdf.text(id, 29, 45.5);
      pdf.setFont('helvetica', 'bold');
      pdf.text('Administrativo', 45, 45.5);

      if (qrData) pdf.addImage(qrData, 'PNG', 68.2, 28, 13.8, 13.8, undefined, 'FAST');
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(4.8);
      pdf.setTextColor(100, 116, 139);
      pdf.text('Control de accesos', 67.2, 44.8);
      pdf.setDrawColor(226, 232, 240);
      pdf.roundedRect(0.25, 0.25, 85.1, 53.48, 2.8, 2.8, 'S');
      pdf.save('credencial_CR80_' + safeFileName(name) + '.pdf');
      toast('PDF CR80 generado ✓', 'ok');
    } catch (e) {
      console.error(e);
      toast('No se pudo generar el PDF CR80. Usa Imprimir CR80 como respaldo.', 'warn');
    }
  }
  ...
  function ensureCredentialPrintStyles() {
    ...
    style.textContent = `
      @media print {
        ...
        html, body { width: 85.6mm !important; height: 53.98mm !important; min-width: 85.6mm !important; min-height: 53.98mm !important; max-width: 85.6mm !important; max-height: 53.98mm !important; margin: 0 !important; padding: 0 !important; overflow: hidden !important; background: #fff !important; }
        body * { visibility: hidden !important; }
        .cred-print, .cred-print * { visibility: visible !important; }
        .cred-print { position: fixed !important; inset: 0 !important; width: 85.6mm !important; height: 53.98mm !important; max-width: 85.6mm !important; max-height: 53.98mm !important; margin: 0 !important; padding: 0 !important; border: none !important; border-radius: 2.8mm !important; box-shadow: none !important; transform: none !important; overflow: hidden !important; background: #fff !important; color: #111827 !important; break-inside: avoid !important; page-break-inside: avoid !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
        ...
        .cred-print .cr80-qr { width: 14mm !important; height: 14mm !important; padding: 1mm !important; border-radius: 1.8mm !important; }
        .cred-print .cr80-name { font-size: 10.4pt !important; line-height: 1.05 !important; }
        .cred-print .cr80-role { font-size: 6.4pt !important; line-height: 1.1 !important; }
        .cred-print .cr80-meta { font-size: 5.4pt !important; line-height: 1.2 !important; }
        .cred-print .cr80-label { font-size: 4.6pt !important; letter-spacing: .08em !important; }
  ...
  function CredentialModal({ user, onClose }) {
    ...
    const avatar = photoOf(user);
    const active = (user.status || 'Activo') === 'Activo';
    return <Modal open width={760} onClose={onClose} title="Credencial CR80" footer={<><button className="btn" onClick={onClose}>Cerrar</button><button className="btn" onClick={() => downloadCredentialPdf(user)}><Icon name="download" size={15} className="btn-ico" />Descargar PDF CR80</button><button className="btn primary" onClick={() => { ensureCredentialPrintStyles(); window.print(); }}><Icon name="print" size={15} className="btn-ico" />Imprimir CR80</button></>}>
      <div className="col gap-12"><div className="faint" style={{ fontSize: 12 }}>Formato físico: CR80 horizontal · 85.6 mm × 53.98 mm. Usa el botón PDF para generar el archivo imprimible.</div><div className="row" style={{ gap: 24, alignItems: 'flex-start', flexWrap: 'wrap' }}>
