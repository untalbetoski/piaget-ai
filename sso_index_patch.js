/* sso_index_patch.js — botón institucional visible en index.html */
(function(){
  var cfgCache=null;
  function apiStatus(){
    if(cfgCache) return Promise.resolve(cfgCache);
    var timeout=new Promise(function(ok){setTimeout(function(){ok({configured:false,domain:'jeanpiaget.mx',timeout:true});},1800);});
    var req=fetch('/api/gws').then(function(r){return r.json();}).then(function(d){cfgCache=d||{};return cfgCache;}).catch(function(){return {configured:false,domain:'jeanpiaget.mx'};});
  ...
  function makeBox(){
    ...
    box.innerHTML='<div style="display:flex;align-items:center;gap:10px;margin:4px 0 12px"><span style="height:1px;background:var(--border);flex:1"></span><span style="font-size:11px;color:var(--text-faint);font-family:var(--font-mono);letter-spacing:.08em">GOOGLE WORKSPACE</span><span style="height:1px;background:var(--border);flex:1"></span></div><div class="gsi-holder" style="display:grid;place-items:center;min-height:44px"><button type="button" class="btn full" style="justify-content:center" disabled>Cargando Google Workspace…</button></div><div class="gsi-domain" style="font-size:11.5px;color:var(--text-faint);text-align:center;margin-top:8px">Dominio permitido: <b>jeanpiaget.mx</b></div>';
    return box;
  }
  function render(holder,cfg){
    cfg=cfg||{configured:false,domain:'jeanpiaget.mx'};
    var domain=cfg.domain||'jeanpiaget.mx';
    var domainEl=holder.parentNode.querySelector('.gsi-domain');
    if(domainEl) domainEl.innerHTML='Dominio permitido: <b>'+domain+'</b>';
    ...
    loadProvider().then(function(){
      window.google.accounts.id.initialize({client_id:cfg.clientId,hosted_domain:domain,callback:function(resp){postCredential(resp.credential).catch(function(e){alert(e.message||e);});}});
      holder.innerHTML='';
  ...
  function mountOne(form){
    ...
    apiStatus().then(function(cfg){render(holder,cfg);});
  }
  function mount(){
    document.querySelectorAll('form').forEach(function(form){
      if(form.querySelector('input[type="email"]')||form.textContent.indexOf('Entrar')>=0) mountOne(form);
    });
  }
