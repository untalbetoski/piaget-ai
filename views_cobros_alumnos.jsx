/* views_cobros_alumnos.jsx — estado de cuenta real por alumno y por concepto */

const CTA_KEY = 'piaget_cuentas_v2_real';
let COB_CUENTAS = (() => { try { return JSON.parse(localStorage.getItem(CTA_KEY) || '{}') || {}; } catch (_) { return {}; } })();

function ctaSaveAll() {
  try { localStorage.setItem(CTA_KEY, JSON.stringify(COB_CUENTAS)); } catch (_) {}
  try { if (window.Store && Store.saveState) Store.saveState(); } catch (_) {}
}
...
function ctaNorm(v) { return ctaClean(v).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''); }
...
function ctaClases() {
  ...
  return ((window.DB && Array.isArray(DB.clases)) ? DB.clases : []).filter(c => c && !ctaIsSeedClass(c) && !deleted.has(c._id) && ctaClean(c.g));
}
function ctaInferNivel(grade) {
  const g = String(grade || '');
  return /sec/i.test(g) ? 'Secundaria' : /^\s*k/i.test(g) ? 'Preescolar' : 'Primaria';
}
...
function ctaPaymentSid(c) {
  if (c && c.sid) return c.sid;
  const who = ctaNorm(c && (c.student || c.alumno || c.name));
  if (!who) return '';
  const found = ctaStudents().find(s => ctaNorm(s.name) === who);
  return found ? found.sid : '';
}
function ctaPaymentKind(c) {
  const explicit = ctaNorm(c && (c.paymentConcept || c.tipo || c.kind));
  if (explicit === 'inscripcion') return 'inscripcion';
  if (explicit === 'extras' || explicit === 'cuota anual') return 'extras';
  if (explicit === 'colegiatura' || explicit === 'anual') return 'colegiatura';
  const txt = ctaNorm(c && (c.concept || c.concepto));
  if (txt.includes('inscrip')) return 'inscripcion';
  if (txt.includes('cuota unica') || txt.includes('cuota anual')) return 'extras';
  if (txt.includes('colegiatura') || txt.includes('mensualidad') || txt.includes('pago anual')) return 'colegiatura';
  return 'otros';
}
function ctaValidPayment(c) {
  const s = ctaNorm(c && c.status);
  return !!c && s !== 'cancelado' && s !== 'anulado';
}
function ctaPaidBySid() {
  const m = {};
  ((window.DB && DB.cobros) || []).forEach(c => {
    if (!ctaValidPayment(c)) return;
    const sid = ctaPaymentSid(c);
    if (!sid) return;
    m[sid] = (m[sid] || 0) + (Number(c.amount) || 0);
  });
  return m;
}
function ctaPaidByConcept(sid) {
  const out = { inscripcion: 0, colegiatura: 0, extras: 0, otros: 0, total: 0 };
  ((window.DB && DB.cobros) || []).forEach(c => {
    if (!ctaValidPayment(c) || ctaPaymentSid(c) !== sid) return;
    const amount = Number(c.amount) || 0;
    const kind = ctaPaymentKind(c);
...
function ctaResolve(stu, paidBySid) {
  ...
  const cargosConcepto = {
    inscripcion: Number(t.inscripcion) || 0,
    colegiatura: Number(colegNeto) || 0,
    extras: Number(t.extras) || 0,
  };
  const pagosRaw = ctaPaidByConcept(stu.sid);
  const pagosConcepto = {
    inscripcion: Math.min(cargosConcepto.inscripcion, pagosRaw.inscripcion),
    colegiatura: Math.min(cargosConcepto.colegiatura, pagosRaw.colegiatura + pagosRaw.otros),
    extras: Math.min(cargosConcepto.extras, pagosRaw.extras),
  };
  const saldosConcepto = {
    inscripcion: Math.max(0, cargosConcepto.inscripcion - pagosConcepto.inscripcion),
    colegiatura: Math.max(0, cargosConcepto.colegiatura - pagosConcepto.colegiatura),
    extras: Math.max(0, cargosConcepto.extras - pagosConcepto.extras),
  };
  const total = cargosConcepto.inscripcion + cargosConcepto.colegiatura + cargosConcepto.extras;
  const registered = paidBySid && paidBySid[stu.sid] != null ? Number(paidBySid[stu.sid]) || 0 : pagosRaw.total;
  const pagado = Math.min(total, registered);
  const saldo = Math.max(0, total - pagado);
  const estatus = saldo <= 0 ? 'liquidado' : pagado > 0 ? 'parcial' : 'sin_pago';
  return { ...stu, plan, beca, t, colegBase, colegNeto, descBeca, total, pagado, saldo, estatus, cargosConcepto, pagosConcepto, saldosConcepto };
}
...
function ctaNextRecibo() {
  const max = (DB.cobros || []).reduce((m, c) => Math.max(m, Number(String(c.recibo || '').replace(/\D/g, '')) || 0), 4900);
  return 'REC-0' + (max + 1);
}
...
function RegistrarPago({ student, paidBySid, onClose, onDone }) {
  ...
  const paidMap = React.useMemo(() => paidBySid || ctaPaidBySid(), [paidBySid, DB.cobros]);
  const groupsOfNivel = React.useMemo(() => [...new Set(ctaStudents().filter(s => (s.nivel || ctaInferNivel(s.group)) === nivel).map(s => s.group).filter(Boolean))], [nivel]);
  const studentsOfGroup = React.useMemo(() => ctaStudents().filter(s => s.group === group).map(s => ctaResolve(s, paidMap)), [group, paidMap]);
  const stu = student ? ctaResolve(student, paidMap) : studentsOfGroup.find(s => s.sid === sid) || null;
  const computed = React.useMemo(() => {
    if (!stu) return { concept: '', amount: 0 };
    if (tipo === 'inscripcion') return { concept: 'Inscripción · ' + stu.name + ' (' + stu.group + ')', amount: stu.saldosConcepto.inscripcion };
    if (tipo === 'extras') return { concept: 'Cuota única anual · ' + stu.name + ' (' + stu.group + ')', amount: stu.saldosConcepto.extras };
    if (tipo === 'anual') return { concept: 'Colegiatura anual · ' + stu.name + ' (' + stu.group + (stu.beca ? ' · beca ' + stu.beca + '%' : '') + ')', amount: stu.saldosConcepto.colegiatura };
  ...
  }, [stu && stu.sid, stu && stu.saldo, tipo, mes]);
  React.useEffect(() => { if (!touchedAmt) setAmount(String(computed.amount || '')); }, [computed.amount, touchedAmt]);
  function save() {
    if (!stu) return toast('Selecciona un alumno', 'warn');
    const n = Number(amount) || 0;
    if (!n) return toast('Captura un monto válido', 'warn');
    Store.add('cobros', {
      recibo: ctaNextRecibo(), sid: stu.sid, student: stu.name, group: stu.group, nivel: stu.nivel,
      family: 'Familia ' + stu.name.split(' ').slice(-1)[0], concept: computed.concept, paymentConcept: tipo,
      amount: n, channel, ref, folio: '', date: new Date().toISOString().slice(0, 10),
    ...
    Store.log('Tesorería', 'registró pago real ' + fmtMoney(n) + ' · ' + stu.name, 'wallet');
  ...
  return <Modal open width={600} onClose={onClose} title={student ? 'Registrar pago · ' + student.name : 'Registrar pago'} footer={<><button className="btn" onClick={onClose}>Cancelar</button><button className="btn primary" onClick={save}><Icon name="check" size={15} className="btn-ico" />Registrar pago real</button></>}>
    ...
    {stu && <div className="row gap-8" style={{ flexWrap: 'wrap', margin: '4px 0 12px' }}>{ctaPlanBadge(stu.plan)}{stu.beca > 0 && <Badge tone="violet">Beca {stu.beca}%</Badge>}<Badge tone="gray">Saldo general {fmtMoney(stu.saldo)}</Badge></div>}
    ...
    {stu && tipo === 'inscripcion' && <div className="faint" style={{ fontSize: 12, marginBottom: 10 }}>Saldo pendiente de inscripción: <b>{fmtMoney(stu.saldosConcepto.inscripcion)}</b></div>}
...
function StudentAccount({ stu, onClose, onChange, onPay }) {
  ...
  const pagos = (DB.cobros || []).filter(c => ctaPaymentSid(c) === stu.sid).slice().sort((a, b) => ((b.date || '') + (b.time || '')).localeCompare((a.date || '') + (a.time || '')));
  ...
  const filas = [
    { k: 'Inscripción', cargo: stu.cargosConcepto.inscripcion, pagado: stu.pagosConcepto.inscripcion, saldo: stu.saldosConcepto.inscripcion, note: 'sin beca' },
    { k: 'Colegiatura del ciclo · ' + cobPlanLabel(stu.plan), cargo: stu.cargosConcepto.colegiatura, pagado: stu.pagosConcepto.colegiatura, saldo: stu.saldosConcepto.colegiatura, note: stu.beca > 0 ? ('base ' + fmtMoney(stu.colegBase) + ' · beca ' + stu.beca + '%') : cobPlanLabel(stu.plan) },
    { k: 'Cuota única anual', cargo: stu.cargosConcepto.extras, pagado: stu.pagosConcepto.extras, saldo: stu.saldosConcepto.extras, note: 'sin beca' },
  ];
...
function EstadoCuenta({ go }) {
  ...
  const groups = React.useMemo(() => {
    ...
    return base.map(g => {
      const first = all.find(s => s.group === g) || {};
      const studs = all.filter(s => s.group === g);
      ...
      return { group: g, nivel: first.nivel || 'Primaria', tone: window.nivelCfg ? window.nivelCfg(first.nivel || 'Primaria').tone : 'blue', n: studs.length, ...agg };
    });
  }, [all]);
...
function ctaAdeudos() {
  ...
  const students = (window.DB && DB.students) || [];
  const key = cobros.length + '|' + students.length + '|' + JSON.stringify(COB_CUENTAS);
...
Object.assign(window, { ctaStudents, ctaResolve, ctaNextRecibo, ctaAdeudos, ctaPaidBySid, ctaPaidByConcept, ctaPaymentKind, ctaEstatusBadge, ctaPlanBadge, ctaSet, ctaGet });
