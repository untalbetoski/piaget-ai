/* views_diario.jsx — Gestión › Diario con datos reales únicamente */

function diarioClean(v) { return String(v || '').trim().replace(/\s+/g, ' '); }
function diarioIsSeedClass(c) { return /^cls-\d+$/i.test(String((c && c._id) || '')); }
function diarioDeletedClassIds() { try { return new Set((DB.settings && DB.settings.deletedClassIds) || []); } catch (_) { return new Set(); } }
...
function diarioRoster(c) {
  ...
  const list = ((window.DB && Array.isArray(DB.students)) ? DB.students : []).filter(s => {
...
function diarioSettings() { window.DB = window.DB || {}; DB.settings = DB.settings || {}; DB.settings.diarioReal = DB.settings.diarioReal || []; return DB.settings.diarioReal; }
function diarioEntries() { return Array.isArray(diarioSettings()) ? diarioSettings().filter(e => e && e._id && e.group && diarioClassOf(e.group)) : []; }
function diarioSave(entries) { DB.settings = DB.settings || {}; DB.settings.diarioReal = entries || []; try { if (window.Store && Store.saveState) Store.saveState(); } catch (_) {} try { window.dispatchEvent(new Event('piaget-diario')); } catch (_) {} }
function diarioUid() { return crypto.randomUUID ? crypto.randomUUID() : 'diario-' + Date.now() + '-' + Math.random().toString(36).slice(2); }
function diarioMaterias(nivel) {
  const ev = window.EV_MATERIAS || {};
  return ev[nivel] || ['Español', 'Matemáticas', 'Ciencias', 'Historia', 'Inglés', 'Educación Física', 'Artes'];
}
function diarioBlank() {
  const c = diarioClasses()[0];
  const nivel = c ? c.nivel : 'Primaria';
  return { _id: '', date: diarioToday(), group: c ? c.g : '', nivel, subject: diarioMaterias(nivel)[0], title: '', avance: '', observaciones: '', acuerdos: '', docente: (DB.user && DB.user.name) || 'Docente' };
}
...
function DiarioModal({ entry, onClose }) {
  ...
  function setGroup(g) { const c = diarioClassOf(g); setForm(f => ({ ...f, group: g, nivel: c ? c.nivel : f.nivel, subject: diarioMaterias(c ? c.nivel : f.nivel)[0] || f.subject })); }
  function save() {
    ...
    const next = existing.some(e => e._id === payload._id) ? existing.map(e => e._id === payload._id ? payload : e) : [payload, ...existing];
    diarioSave(next);
...
function Diario({ go }) {
  ...
  return <div className="content-inner">
    <PageHead eyebrow="Gestión" title="Diario" desc={entries.length + ' entradas reales · sin registros demo'}>
      <button className="btn" onClick={() => go && go('clases')}><Icon name="layers" size={15} className="btn-ico" />Clases</button>
      <button className="btn primary" onClick={() => setModal({})}><Icon name="plus" size={15} className="btn-ico" />Nueva entrada</button>
    </PageHead>
